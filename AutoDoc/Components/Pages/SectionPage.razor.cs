    ...
    public partial class SectionPage
    {
        ...
        private async Task LoadSectionsAsync()
        {
            try
            {
                ...
                do
                {
                    SectionStatusType? statusFilter = null;
                    if (_selectedStatuses != null && _selectedStatuses.Count == 1)
                    {
                        statusFilter = _selectedStatuses.First();
                    }

                    pagedList = await SectionsService.GetSectionsAsync(
                        groupId,
                        _searchTerm,
                        statusFilter,
                        _currentOffset,
        ...
        public async Task LoadGroupSectionsAsync()
        {
            try
            {
                var result = await GroupService.GetGroupsAsync(null, "ACTIVE", 0, 0);
                group = result.Items?.FirstOrDefault(g => g.ID == groupId) ?? new SectionGroupGetDTO();
            }
            catch (Exception)
            {
